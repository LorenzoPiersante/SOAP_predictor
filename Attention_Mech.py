...
class self_attn_init(nn.Module):
    ...
    def forward(self, nodes, centre):
        ...
        T_nodes = torch.tensor(nodes, requires_grad = False, dtype = torch.float, device = my_device)
        centre = torch.tensor(centre, requires_grad = False, dtype = torch.float, device = my_device)
        T_centre = torch.zeros((T_nodes.size(dim=0), centre.size(dim=0)), requires_grad = False, dtype = torch.float, device = my_device)
        ...
        #pre-allocate output
        outputs = torch.zeros((T_nodes.size(dim=0), self.output_size), requires_grad = False, dtype = torch.float, device = my_device)
        #compute output row by row
        for n in range(T_nodes.size(dim=0)):
            #create key-queries input
            ref_key =  torch.zeros((T_nodes.size(dim=0), self.output_size), requires_grad = False, dtype = torch.float, device = my_device)
...
class SOAP_dec(nn.Module):
    ...
    def forward(self, nodesL, nodesH, centre, system_vect):
        #generate concatenated input (must speecify no gradient and dtype = float)
        nodesL = torch.tensor(nodesL, requires_grad = False, dtype = torch.float, device = my_device)
        nodesH = torch.tensor(nodesH, requires_grad = False, dtype = torch.float, device = my_device)
        centre = torch.tensor(centre, requires_grad = False, dtype = torch.float, device = my_device)
        T_centreL = torch.zeros((nodesL.size(dim=0), centre.size(dim=0)), requires_grad = False, dtype = torch.float, device = my_device)
        for i in range(nodesL.size(dim=0)):
            T_centreL[i, :] = centre
        T_centreH = torch.zeros((nodesH.size(dim=0), centre.size(dim=0)), requires_grad = False, dtype = torch.float, device = my_device)
        for i in range(nodesH.size(dim=0)):
            T_centreH[i, :] = centre
        T_systemL = torch.zeros((nodesL.size(dim=0), system_vect.size(dim=0)), requires_grad = False, dtype = torch.float, device = my_device)
        for i in range(nodesL.size(dim=0)):
            T_systemL[i, :] = system_vect
        T_systemH = torch.zeros((nodesH.size(dim=0), system_vect.size(dim=0)), requires_grad = False, dtype = torch.float, device = my_device)
        for i in range(nodesH.size(dim=0)):
        ...
        #pre-allocate output
        outputs = torch.zeros((nodesL.size(dim=0), self.output_size), requires_grad = False, dtype = torch.float, device = my_device)
        #compute output row by row
        for n in range(nodesL.size(dim=0)):
            #create key-queries input
            ref_key = torch.zeros((nodesH.size(dim=0), self.output_size), device = my_device)
