...
class self_attn_init(nn.Module):
    ...
    def forward(self, nodes, centre):
        ...
        T_centre = torch.zeros((T_nodes.size(dim=0), centre.size(dim=0)), requires_grad = False, dtype = torch.float)
...
class SOAP_dec(nn.Module):
    ...
    def forward(self, nodesL, nodesH, centre, system_vect):
        ...
        centre = torch.tensor(centre, requires_grad = False, dtype = torch.float)
        T_centreL = torch.zeros((nodesL.size(dim=0), centre.size(dim=0)), requires_grad = False, dtype = torch.float)
        for i in range(nodesL.size(dim=0)):
            T_centreL[i, :] = centre
        T_centreH = torch.zeros((nodesH.size(dim=0), centre.size(dim=0)), requires_grad = False, dtype = torch.float)
        for i in range(nodesH.size(dim=0)):
            T_centreH[i, :] = centre
        T_systemL = torch.zeros((nodesL.size(dim=0), system_vect.size(dim=0)), requires_grad = False, dtype = torch.float)
        for i in range(nodesL.size(dim=0)):
            T_systemL[i, :] = system_vect
        T_systemH = torch.zeros((nodesH.size(dim=0), system_vect.size(dim=0)), requires_grad = False, dtype = torch.float)
        for i in range(nodesH.size(dim=0)):
