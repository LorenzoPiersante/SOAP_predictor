...
class hidden_state_init(nn.Module):
    ...
    - output_size : size of the initialised hidden state vector
...
class GRU_graph_enc(nn.Module):
   ...
   - h_0 : the initialisation outputs from hidden_state_init
    ...
    def __init__(self, input_size, hidden_size, iteration):
        super().__init__()
        self.gru = nn.GRU(input_size, hidden_size, batch_first=True)
        self.state = iteration

    def forward(self, graph, h_0):

        # Account for addition of first node
        if self.state and len(h_0) == 1:
            return h_0
