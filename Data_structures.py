...
# Add a dir_fetcher function

# 31-07-2022

# Modify the xyz_manager() class to create noisy inputs for the MoleculeGraph object at the N-1-th iteration of the model. This was done in an attempt to speed up the model execution.
...
def dir_fetcher(target_folder):
    ...
    path2dir = parent_path.joinpath(molecule_folder, folders[target_folder])
...
class xyz_manager:
    ...
    Returns initilaisation variables used in the definition of the MoleculeGraph object
    ...
    def __init__(self, xyz_file, shuffle):
            ...
            for n in range(len(self.atoms_list)): #loop over all atoms
                ...
                for x in envs: #loop over all environments
                    ...
                    if x[0] == x[1]: #SS
                        ...
                        SOAP_atom_dict[x] = torch.tensor(SSOAP_reshaping(V), requires_grad = False, dtype = torch.float, device = my_device).detach()
                        ...
                        SOAP_atom_dict[x] = torch.tensor(V, requires_grad = False, dtype = torch.float, device = my_device).detach()
                    
                SOAP_vectors.append(SOAP_atom_dict)
            
            return SOAP_vectors
            
        def SOAPs_input(self, ordering):
            '''
            This function produces the SOAP dictionaries and the set of atomic species up to iteration N-1.
            The SOAP vectors are randomised through the introduction of white noise.
            
            Returns:
            - SOAP_vectors: SOAP dictionaries with introduction of white noise
            - species_in_mol: set of chem species
            '''
            
            #Create intermediate xyz file
            fl = open(self.file)
            fl = fl.readlines()
            header = fl[0:2]
            atoms_xyz = fl[2:-1]
            #reorder atoms in xyz file according to same shuffling
            reordered_atoms_xyz = [x for x in atoms_xyz]
            reordered_atoms_xyz = reordered_atoms_xyz[0:len(reordered_atoms_xyz)-1]
            #modify header to have correct number of atoms
            header[0] = str(len(reordered_atoms_xyz)) + "\n"
            #create new .xyz file for the molecule
            new_xyz = open("interim_mol.xyz", "w")
            new_xyz.writelines(header+reordered_atoms_xyz)
            new_xyz.close()
            
            SOAP_vectors = []
            
            #create a molecule object
            mol = ase.io.read("interim_mol.xyz")
            #get the atomic species
            species_in_mol = set(mol.get_chemical_symbols()) #not sorted according to Z
            #initialise SOAP object
            soap = SOAP(
            ...
            #compute SOAP, output is an ndarray, each row corresponding to a different atom
            #the output will be in standard SOAP order despite the input being shuffled
            SOAP_array = soap.create(mol)
            ...
            for n in range(len(self.atoms_list_N_1)): #loop over all atoms
                SOAP_atom_vect = SOAP_array[n]
                SOAP_atom_dict = {}
                ...
                for x in envs: #loop over all environments
                    ...
                    if x[0] == x[1]: #SS
                        ...
                        #apply white noise
                        mean_SOAP = np.mean(V)
                        white_noise = mean_SOAP*np.random.randn(16, 4)
                        V = V + white_noise
                        #convert to tensor and add to dictionary
                        SOAP_atom_dict[x] = torch.tensor(V, requires_grad = False, dtype = torch.float, device = my_device).detach()
            ...
            return SOAP_vectors, species_in_mol
        
        def nodes_info_input(self):
            '''
            This function creates the node by class and node encoding input at the end of iteration N-1.

            Returns:
            - node_by_class: as given at iteration N-1
            - init_node_enc: as given at iteration N-1
            '''
            ...
            for atom in self.atoms_list_N_1:


                if atom.atom_class in nodes_by_class:
                    nodes_by_class[atom.atom_class] = np.concatenate((nodes_by_class[atom.atom_class], np.array(atom.position).reshape(1, 3)), axis=0)
                else:
                    nodes_by_class[atom.atom_class] = np.array(atom.position).reshape(1, 3)

                #concatenate the two lists, position + one-hot class
                init_vect = atom.position + atom_class_dict[atom.atom_class]
                #convert it to tensor
                init_vect = torch.tensor(init_vect, requires_grad = False, dtype = torch.float, device = my_device)
                #append it to list of initial state vectors
                init_node_enc.append(init_vect)

            return nodes_by_class, init_node_enc
        ...
        #label SOAP dictionaries
        self.SOAPs_list = SOAPs_label(self)
        ...
        #INPUTS ITERATION N-1
        #input atoms list
        self.atoms_list_N_1 = self.atoms_list[0:len(self.atoms_list)-1]
        #input SOAPs and chem species
        result_iter_N_1 = SOAPs_input(self, ordering)
        SOAPs_N_1 = result_iter_N_1[0]
        chem_envs = result_iter_N_1[1]
        self.SOAPs_N_1 = SOAPs_N_1
        self.species = chem_envs
        
        #input node information
        node_info_N_1 = nodes_info_input(self)
        nodes_by_class = node_info_N_1[0]
        init_node_enc = node_info_N_1[1]
        self.nodes_by_class = nodes_by_class
        self.init_node_enc = init_node_enc
...
class MoleculeGraph:
    ...
    - SOAP_list : list of SOAPs
    produced by xyz_manager(), the order of the atomic indeces is the same in both lists
    ...
    def __init__(self, atoms_list, SOAPs_list):
        #initialise the object as an empty container.
        #NETWORK objects
        self.atoms = [] #x in xyz_manager()
        self.species = set() #x in xyz_manager()
        self.SOAPs = [] #x in xyz_manager()
        self.nodes_by_class = {} #x in xyz_manager()
        
        #these are tensor objects
        self.init_node_enc = [] #x in xyz_manager()
        self.SOAP_inputs = None #this will be re-initialised at each iteration, and filled with GRU tensor inputs
        
        #LABEL objects
        self.atom_labels = atoms_list
        self.SOAP_labels = SOAPs_list
    ...
    def add_atom(self, new_atom):
        ...
        if bool(new_env_tuples):
            if len(self.atoms) == 1: #handle addition of first atom
                ...
                for x in new_env_tuples:
                    self.SOAPs[0][x] = torch.zeros((size_l_IS, (l_max+1)), requires_grad = False, device = my_device)
            else:
                #first add new environments to pre-existing atom disctionaries
                for n in range(len(self.atoms)-1):
                    for x in new_env_tuples:
                        self.SOAPs[n][x] = torch.zeros((size_l_IS, (l_max+1)), requires_grad = False, device = my_device)
                #add new atom dictionary, must loop over all env tuples
                self.SOAPs.append({})
                for x in new_envs:
                    self.SOAPs[-1][x] = torch.zeros((size_l_IS, (l_max+1)), requires_grad = False, device = my_device)
        else:
        ...
        for n in range(len(self.atoms)-1): #iterate over all atoms, except for the new addition
            ...
            node_input = torch.zeros((len(current_envs), int(size_l_IS*(l_max+1))), requires_grad = False, dtype = torch.float, device = my_device)

            for x, y in zip(current_envs, range(len(current_envs))):
                #extract desired env
                X = self.SOAPs[n][x].detach()
                #split it into colums
                x_1, x_2, x_3, x_4 = torch.split(X, [1, 1, 1, 1], dim=1)
                #stack them into a single tensor along dim=0
                SOAP_env = torch.cat((x_1, x_2, x_3, x_4), dim=0)
                #remove dummy dim
                SOAP_env = torch.squeeze(SOAP_env, dim=1)
